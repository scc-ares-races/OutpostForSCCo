...
for REPO in scc-ares-races/pack-it-forms "$@"; do
  ...
  if [ ! -e "$FORMS" ]; then
    ...
    (cd "$FORMS" && git checkout vSCCo.53)
    rm -rf "$FORMS"/.git*
  fi
  "$FORMS"/resources/integration/scco/build.sh ./buildInstaller.sh "$VersionMajor.$VersionMinor$VersionBeta" || exit $?
done
